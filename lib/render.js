...
var renderNode = exports.renderNode = function renderNode(node, inlineRenderers, entityRenderers, styleRenderers, entityMap, options, keyGenerator) {
  ...
  if (node.style && inlineRenderers[node.style]) {
    return inlineRenderers[node.style]((0, _checkJoin2.default)(children, options), { key: keyGenerator(), style: node.style });
  } else if (node.style && inlineRenderers.$unmatched) {
    return inlineRenderers.$unmatched((0, _checkJoin2.default)(children, options), {
      key: keyGenerator(),
      style: node.style
    });
  }
